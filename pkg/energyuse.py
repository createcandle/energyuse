    ...
    def add_from_config(self):
        ...
        try:
            ...
            if 'Data retention months' in config:
                ...
                self.data_retention_months
        
        except:
            print("Error loading data from settings")
...
#
    ...
    def clock(self):
        ...
        if self.DEBUG:
            print("CLOCK INIT.. Will sleep seconds: ")
        ...
        while clock_active and self.running: # and self.player != None
            try:
                ...
                #long_sleep = int(time.time()) % 86400
            
                #if self.DEBUG:
                #    print("Will sleep seconds: " + str(long_sleep))
                ...
                # A new day
                #
                #print(str(self.test_counter ))
                if current_time.day != self.persistent_data['last_day']: # or self.test_counter > 5:
                    self.test_counter = 0
                    if self.DEBUG:
                    ...
                    try:
                        ...
                        self.got_fresh_things_list = False
                        for x in range(6):
                            p#rint("in for loop to get fresh things")
        ...
        try:
            ...
            if self.previous_hour_total == None:
                ...
                if self.previous_hour_day_delta == None:
                    if self.DEBUG:
                        print("remembering previous_hour_day_delta")
                    self.previous_hour_day_delta = day_delta # to kwh of the day so far, to check if that today-so-far value has grown over the last hour.
                else:
                    if self.DEBUG:
                        print("hour-delta. day_delta: " + str(day_delta))
                        print("hour-delta. self.previous_hour_day_delta: " + str(self.previous_hour_day_delta))
                    if day_delta > self.previous_hour_day_delta:
                        hourly_delta = day_delta - self.previous_hour_day_delta
                        if self.DEBUG:
                            print("hourly change: " + str(hourly_delta))
                        self.set_value_on_thing('lasthour',hourly_delta)
                    else:
                        if self.DEBUG:
                            print("no energy used this hour?")
